...
def load_data():
    profile_file_path = os.path.join(FILES_DIR, "profile_service_response.json")
    posts_file_path = os.path.join(FILES_DIR, "posts_management_service_response.json")
    moderation_file_path = os.path.join(FILES_DIR, "moderation_service_response.json")

    with open(profile_file_path, encoding="utf-8") as profile_file:
        profile_json = json.load(profile_file)

    with open(posts_file_path, encoding="utf-8") as posts_file:
        posts_json = json.load(posts_file)

    with open(moderation_file_path, encoding="utf-8") as moderation_file:
...
def write_plantuml_file(profile_data, posts_data, moderation_data):
    output_path = os.path.join(FILES_DIR, "plantuml.txt")
